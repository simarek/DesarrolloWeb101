...
class categora extends Modelo{
  ...
  */
  ...
  public function store($data){ // array[]
    $consulta = "INSERT INTO $this->nombre_tabla ('cur_descripcion') value ('".$data['cur_descripcion']."')";
    $resutado = $this->de->query($consulta); // realizando la consulta a la BD
    if(!$resultado){
  ...
  public function update($id, $data){ // array[]
    $consulta = "UPDATE $this->nombre_tabla SET cur_descripcion = '".$data['cur_descripcion']."' WHERE $this_id = ".$id;
    $resutado = $this->de->query($consulta); // realizando la consulta a la BD
    if(!$resultado){
