...
class categora extends Modelo{
  ...
  */
  ...
  public function __construct(){
    ...
    $this->id= 'exp_id';
    $this->nombr_tabla='experiencia_laboral';
  }
  ...
  public function store($data){ // array[]
    $consulta = "INSERT INTO $this->nombre_tabla ('exp_descripcion') value ('".$data['exp_descripcion']."')";
    $resutado = $this->de->query($consulta); // realizando la consulta a la BD
    if(!$resultado){
  ...
  public function update($id, $data){ // array[]
    $consulta = "UPDATE $this->nombre_tabla SET exp_descripcion = '".$data['exp_descripcion']."' WHERE $this_id = ".$id;
    $resutado = $this->de->query($consulta); // realizando la consulta a la BD
    if(!$resultado){
