...
class categora extends Modelo{
  ...
  */
  ...
  public function store($data){ // array[]
    $consulta = "INSERT INTO $this->nombre_tabla (usu_usr) value ('".$data['usu_usr']."')";
    $resutado = $this->de->query($consulta); // realizando la consulta a la BD
    if(!$resultado){
  ...
  public function update($id, $data){ // array[]
    $consulta = "UPDATE $this->nombre_tabla SET usu_usr = '".$data['usu_usr']."' WHERE $this_id = ".$id;
    $resutado = $this->de->query($consulta); // realizando la consulta a la BD
    if(!$resultado){
