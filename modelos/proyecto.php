...
class categora extends Modelo{
  ...
  */
  ...
  public function store($data){ // array[]
    $consulta = "INSERT INTO $this->nombre_tabla ('pro_titulo') value ('".$data['pro_titulo']."')";
    $resutado = $this->de->query($consulta); // realizando la consulta a la BD
    if(!$resultado){
  ...
  public function update($id, $data){ // array[]
    $consulta = "UPDATE $this->nombre_tabla SET pro_titulo = '".$data['pro_titulo']."' WHERE $this_id = ".$id;
    $resutado = $this->de->query($consulta); // realizando la consulta a la BD
    if(!$resultado){
