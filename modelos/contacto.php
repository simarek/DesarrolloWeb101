...
class categora extends Modelo{
  ...
  */
  ...
  public function store($data){ // array[]
    $consulta = "INSERT INTO $this->nombre_tabla ('con_nombre') value ('".$data['con_nombre']."')";
    $resutado = $this->de->query($consulta); // realizando la consulta a la BD
    if(!$resultado){
  ...
  public function update($id, $data){ // array[]
    $consulta = "UPDATE $this->nombre_tabla SET con_nombre = '".$data['con_nombre']."' WHERE $this_id = ".$id;
    $resutado = $this->de->query($consulta); // realizando la consulta a la BD
    if(!$resultado){
